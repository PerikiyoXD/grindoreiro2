#!/usr/bin/env python3
"""Batch HTML Report Generator for Grindoreiro Analysis Results."""
...
class BatchHTMLReportGenerator:
    """Batch HTML report generator with deduplication and enhanced features."""
    ...
    def _get_js_scripts(self) -> str:
        ...
        function exportToJSON() {
            const data = {
                ...
                batchData: window.batchData || {}
            };
            const blob = new Blob([JSON.stringify(data, null, 2)], {type: 'application/json'});
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = 'batch_analysis_report.json';
            a.click();
        ...
        function exportToCSV() {
            ...
            let csv = 'Sample,Files,URLs,Strings,Threat_Level\\n';
            const samples = window.batchData || [];
            samples.forEach(sample => {
                csv += `${sample.name},${sample.files_count},${sample.urls_count},${sample.strings_count},${sample.threat_level}\\n`;
            });
    ...
    def generate_batch_report(self, json_files: List[Path], output_dir: Path) -> str:
        """Generate batch HTML report from multiple JSON files."""
        batch_data = []
        ...
        for json_file in json_files:
            try:
                ...
                batch_data.append({
                    ...
                    'path': str(json_file),
                    'html_path': str(json_file.with_suffix('.html')),
                    'size': sample_info.get('size', 0),
                    'files_count': len(file_analysis.get('extracted_files', [])),
                    ...
                    'analysis_time': data.get('session_info', {}).get('analysis_start', 'N/A')
                })
        ...
        for item in batch_data:
            json_file = Path(item['path'])
            try:
                with open(json_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)

                html_content = self._generate_single_report(data)
                ...
                print(f"Error generating HTML for {json_file}: {e}")

        # Generate batch summary report
        return self._generate_batch_summary_html(batch_data)

    def _generate_single_report(self, json_data: Dict[str, Any]) -> str:
        """Generate HTML report for a single analysis (simplified version)."""
        ...
        # Deduplicate data
        deduplicated_urls = self._deduplicate_urls(content_analysis.get('urls_found', []))
        deduplicated_files = self._deduplicate_files(file_analysis.get('extracted_files', []))
...
<body>
    <div class="container">
        {self._generate_header(sample_info, session_info, file_analysis, content_analysis)}
        {self._generate_overview(file_analysis, content_analysis)}
        {self._generate_tabs(deduplicated_files, deduplicated_urls, processing_stages, content_analysis)}
    ...
    def _generate_batch_summary_html(self, batch_data: List[Dict]) -> str:
        """Generate batch summary HTML report."""
        total_samples = len(batch_data)
        total_files = sum(item['files_count'] for item in batch_data)
        total_urls = sum(item['urls_count'] for item in batch_data)
        total_strings = sum(item['strings_count'] for item in batch_data)

        threat_levels = {}
        for item in batch_data:
            level = item['threat_level'].upper()
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Batch Malware Analysis Report</title>
    <style>{self.css_styles}</style>
</head>
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="subtitle">Analysis summary for {total_samples} samples</div>
            ...
            <div class="info-grid">
                <div class="info-card">
                    ...
                    <p><strong>Total Strings:</strong> {total_strings:,}</p>
                </div>

                <div class="info-card">
                    <h3>🎯 Threat Distribution</h3>
                    {''.join(f'<p><strong>{level}:</strong> {count} samples</p>' for level, count in threat_levels.items())}
                </div>

                <div class="info-card">
                    <h3>⏱️ Analysis Details</h3>
                    <p><strong>Report Generated:</strong> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
                    <p><strong>Analysis Tool:</strong> Grindoreiro v2.0</p>
                </div>
            </div>
        </div>
        ...
        <div class="batch-summary">
            ...
            <div class="batch-grid">
                {''.join(self._generate_batch_item_html(item) for item in batch_data)}
            </div>
        </div>
    ...
    <script>
        window.batchData = {json.dumps(batch_data, indent=2)};
        {self.js_scripts}
    </script>
</body>
</html>
    ...
    def _generate_batch_item_html(self, item: Dict) -> str:
        """Generate HTML for a single batch item."""
        threat_color = {
            'HIGH': '#dc3545',
            'MEDIUM': '#ffc107',
            'LOW': '#28a745',
            'UNKNOWN': '#6c757d'
        }.get(item['threat_level'].upper(), '#6c757d')
        ...
        <div class="batch-item">
            ...
            <div class="meta">
                ...
                <p><strong>Files:</strong> {item['files_count']} | <strong>URLs:</strong> {item['urls_count']} | <strong>Strings:</strong> {item['strings_count']:,}</p>
                <p><strong>Threat Level:</strong> <span style="color: {threat_color}; font-weight: bold;">{item['threat_level'].upper()}</span></p>
                <p><strong>Family:</strong> {item['malware_family']}</p>
            </div>
            <div class="actions">
                <a href="{Path(item['html_path']).name}" class="btn" target="_blank">View Report</a>
                <button class="btn" onclick="copyToClipboard('{item['name']}')">Copy Name</button>
            </div>
        </div>
        """

    # Include the same methods from the single report generator
    def _generate_header(self, sample_info: Dict, session_info: Dict, file_analysis: Dict, content_analysis: Dict) -> str:
        ...
        <div class="header">
            ...
            <div class="info-grid">
                ...
                <div class="info-card">
                    <h3>🎯 Quick Status</h3>
                    <p><strong>Files Processed:</strong> {len(file_analysis.get('extracted_files', []))}</p>
                    <p><strong>URLs Found:</strong> {len(content_analysis.get('urls_found', []))}</p>
                    <p><strong>Strings:</strong> {content_analysis.get('strings_count', 0):,}</p>
                </div>
            </div>
        </div>
        """
...
def main():
    """Main function to generate batch HTML reports."""
    parser = argparse.ArgumentParser(description='Generate batch HTML reports from analysis JSON files')
    parser.add_argument('-i', '--input-dir', default='./data/output',
                       help='Input directory containing JSON files (default: ./data/output)')
    parser.add_argument('-o', '--output-dir', default='./data/output',
                       help='Output directory for HTML files (default: ./data/output)')
    parser.add_argument('--batch-report', default='batch_report.html',
                       help='Filename for batch summary report (default: batch_report.html)')
    ...
    # Find all JSON files
    json_files = list(input_dir.glob('*_analysis.json'))
    if not json_files:
    ...
    # Generate reports
    generator = BatchHTMLReportGenerator()
    batch_html = generator.generate_batch_report(json_files, output_dir)

    # Save batch report
    batch_report_path = output_dir / args.batch_report
    with open(batch_report_path, 'w', encoding='utf-8') as f:
        f.write(batch_html)

    print(f"Batch report generated: {batch_report_path}")
    print(f"Individual reports generated in: {output_dir}")
